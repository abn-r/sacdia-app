...
final selectedMonthProvider =
    StateNotifierProvider.autoDispose<SelectedMonthNotifier, SelectedMonth>(
...
class TransactionFormNotifier extends StateNotifier<TransactionFormState> {
  ...
  final UpdateTransaction _update;
  final Ref _ref;

  TransactionFormNotifier({
    required CreateTransaction create,
    required UpdateTransaction update,
    required Ref ref,
  })  : _create = create,
        _update = update,
        _ref = ref,
  ...
  Future<bool> save({
    ...
    if (existingId != null) {
      // Update
      final result = await _update(UpdateTransactionParams(
      ...
      return result.fold(
        ...
        (_) {
          ...
          _ref.invalidate(financeMonthProvider);
      ...
      // Create
      final result = await _create(CreateTransactionParams(
        clubId: clubId,
        categoryId: categoryId,
        amount: amount,
        description: description,
...
final transactionFormNotifierProvider = StateNotifierProvider.autoDispose<
  ...
  (ref) => TransactionFormNotifier(
    create: ref.read(createTransactionUseCaseProvider),
    update: ref.read(updateTransactionUseCaseProvider),
    ref: ref,
  ),
);
